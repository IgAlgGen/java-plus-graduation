...
- В конфигурации core-сервисов задан `server.port=0`, поэтому они стартуют на случайных внутренних портах и доступны через Eureka/Gateway, а не напрямую с host-машины.
...
| --- | --- | --- |
| `SERVER_PORT` | `config-server` | Фиксирует порт Config Server на `8888` внутри compose. |
...
| `user-service` | random container port | `server.port=0`, доступ через Gateway/Eureka. |
| `event-service` | random container port | `server.port=0`, доступ через Gateway/Eureka. |
| `request-service` | random container port | `server.port=0`, доступ через Gateway/Eureka. |
| `feature-service` | random container port | `server.port=0`, доступ через Gateway/Eureka. |
...
- Business-сервисы в compose запускаются на случайных внутренних портах (`server.port=0`), поэтому прямой доступ с host-машины не предусмотрен.
- `config-server` в compose слушает `8888`, но порт не опубликован наружу.
- В docker-compose для некоторых app-сервисов используется `condition: service_started`, а не полноценная проверка `/actuator/health`; сразу после запуска возможны временные ошибки маршрутизации.

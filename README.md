...
| `category-service` | `core/category-service` | Категории событий и внутренний API проверки/получения категорий. |
| `compilation-service` | `core/compilation-service` | Подборки событий. Получает краткие данные событий из `event-service`. |
| `comment-service` | `core/comment-service` | Комментарии и модерация комментариев. Проверяет пользователей и события через Feign. |
...
- `event-service -> category-service` через OpenFeign: проверка и получение категорий.
...
- `category-service -> event-service` через OpenFeign: проверка использования категории событиями перед удалением.
- `compilation-service -> event-service` через OpenFeign: проверка событий и получение кратких данных событий для подборок.
- `comment-service -> user-service` через OpenFeign: проверка автора комментария.
- `comment-service -> event-service` через OpenFeign: проверка событий.
...
| PostgreSQL 16.1 | Docker БД для user/event/request/category/compilation/comment/stats сервисов. |
...
4. `user-service`, `category-service`, `stats-server`
5. `event-service`, `request-service`, `compilation-service`, `comment-service`
6. `gateway-server`
...
| `USER_SERVICE_ID` | `event-service`, `request-service`, `comment-service` | Имя user-service для Feign. |
| `EVENT_SERVICE_ID` | `request-service`, `category-service`, `compilation-service`, `comment-service` | Имя event-service для Feign. |
| `REQUEST_SERVICE_ID` | `event-service` | Имя request-service для Feign. |
| `CATEGORY_SERVICE_ID` | `event-service` | Имя category-service для Feign. |
...
| `category-service` | `8094` container only | Порт задан через `SERVER_PORT` в compose; внешний доступ идет через Gateway/Eureka. |
| `compilation-service` | `8095` container only | Порт задан через `SERVER_PORT` в compose; внешний доступ идет через Gateway/Eureka. |
| `comment-service` | `8096` container only | Порт задан через `SERVER_PORT` в compose; внешний доступ идет через Gateway/Eureka. |
...
| `category-db` | `5432` container only | PostgreSQL БД `ewm-category`. |
| `compilation-db` | `5432` container only | PostgreSQL БД `ewm-compilation`. |
| `comment-db` | `5432` container only | PostgreSQL БД `ewm-comment`. |
...
| `/categories/**`, `/admin/categories/**` | `category-service` |
| `/compilations/**`, `/admin/compilations/**` | `compilation-service` |
| `/comments/**`, `/admin/comments/**` | `comment-service` |
| `/events/*/comments` | `comment-service` |
| `/admin/users/*/events/*/comments`, `/admin/users/*/comments/*` | `comment-service` |
...
| `GET` | `/categories/{catId}` | `category-service` |
| `POST` | `/admin/categories` | `category-service` |
| `PATCH` | `/admin/categories/{catId}` | `category-service` |
| `DELETE` | `/admin/categories/{catId}` | `category-service` |
| `GET` | `/compilations` | `compilation-service` |
| `GET` | `/compilations/{compId}` | `compilation-service` |
| `POST` | `/admin/compilations` | `compilation-service` |
| `PATCH` | `/admin/compilations/{compId}` | `compilation-service` |
| `DELETE` | `/admin/compilations/{compId}` | `compilation-service` |
| `GET` | `/events/{eventId}/comments` | `comment-service` |
| `GET` | `/comments` | `comment-service` |
| `POST` | `/admin/users/{userId}/events/{eventId}/comments` | `comment-service` |
| `PATCH` | `/admin/users/{userId}/comments/{commentId}` | `comment-service` |
| `DELETE` | `/admin/users/{userId}/comments/{commentId}` | `comment-service` |
| `PATCH` | `/admin/comments/{commentId}/approve` | `comment-service` |
| `PATCH` | `/admin/comments/{commentId}/reject` | `comment-service` |
...
| `category-db` | `ewm-category` | `category` | `category-service` | `core/category-service/src/main/resources/schema.sql` |
| `compilation-db` | `ewm-compilation` | `compilation` | `compilation-service` | `core/compilation-service/src/main/resources/schema.sql` |
| `comment-db` | `ewm-comment` | `comment` | `comment-service` | `core/comment-service/src/main/resources/schema.sql` |
...
docker compose logs -f compilation-service
